...
// ---- 颗粒 (43-54)：顺序必须与 develop_uniforms.dart 的 setFloat 一致 ----
uniform float uGrainAmount;        // 43  强度 0-1
uniform float uGrainSize;          // 44  尺寸(相对) 0.1-10
uniform float uGrainShadowThr;     // 45  阴影阈值 0-1
uniform float uGrainHighlightThr;  // 46  高光阈值 0-1
uniform float uGrainShadowStr;     // 47  阴影强度比
uniform float uGrainHighlightStr;  // 48  高光强度比
uniform float uGrainShadowSize;    // 49  阴影尺寸比
uniform float uGrainHighlightSize; // 50  高光尺寸比
uniform float uGrainRedRatio;      // 51  红通道系数
uniform float uGrainBlueRatio;     // 52  蓝通道系数
uniform float uGrainCorrelation;   // 53  通道相关性
uniform float uGrainColorPreserve; // 54  色彩保护
...
// ============================================================================
// Grain 颗粒噪声
// Box-Muller 高斯 + 影调分区 + 通道差异 + 色彩保护
// ============================================================================
// Lut2Photo 多重哈希随机数
float grainRandom(vec2 co, float seed) {
    vec2 p = co + seed;
    float h1 = fract(sin(dot(p, vec2(127.1, 311.7))) * 43758.5453123);
    float h2 = fract(sin(dot(p, vec2(269.5, 183.3))) * 43758.5453123);
    return fract(h1 + h2);
}
// Box-Muller 高斯（均值 0，标准差 1）
float grainGauss(vec2 uv, float seed) {
    float u1 = max(grainRandom(uv, seed), 0.0001);
    float u2 = grainRandom(uv, seed + 0.5);
...
float grainNoise(vec2 p, float seed) {
    ...
    vec2 f = fract(p);
    f = f * f * (3.0 - 2.0 * f);
    float a = grainGauss(i,                  seed);
    float b = grainGauss(i + vec2(1.0, 0.0), seed);
    float c = grainGauss(i + vec2(0.0, 1.0), seed);
    float d = grainGauss(i + vec2(1.0, 1.0), seed);
    return mix(mix(a, b, f.x), mix(c, d, f.x), f.y);
}
// 影调过渡（阴影比 → 中间调 1.0 → 高光比，过渡宽度 0.04）
float grainToneRatio(float l, float thrLo, float thrHi, float lo, float hi) {
    const float tw = 0.04;
    if (l < thrLo - tw) return lo;
    if (l < thrLo + tw) return mix(lo, 1.0, smoothstep(0.0, 1.0, (l - thrLo + tw) / (2.0 * tw)));
    if (l < thrHi - tw) return 1.0;
    if (l < thrHi + tw) return mix(1.0, hi, smoothstep(0.0, 1.0, (l - thrHi + tw) / (2.0 * tw)));
    return hi;
}

vec3 applyGrain(vec3 c, vec2 fragCoord, vec2 imgSize) {
    if (uGrainAmount < 0.001) return c;
    const float SEED = 73.0;    // 固定种子
    const float REF  = 1000.0;  // 参考高度（分辨率无关）
    const float CELL = 1.6;     // grainSize=1 的格子像素：小=细脆 大=粗
    const float AMP  = 0.15;    // 主幅度：颗粒整体强弱

    // Rec.601 亮度
    float l = dot(c, vec3(0.299, 0.587, 0.114));
    float strRatio  = grainToneRatio(l, uGrainShadowThr, uGrainHighlightThr,
                                     uGrainShadowStr,  uGrainHighlightStr);
    float sizeRatio = grainToneRatio(l, uGrainShadowThr, uGrainHighlightThr,
                                     uGrainShadowSize, uGrainHighlightSize);

    // 格子大小随分辨率缩放
    float cellPx = max(uGrainSize * sizeRatio * CELL * (imgSize.y / REF), 0.5);
    vec2 p = fragCoord / cellPx;

    // 基础噪声 + 三通道（按相关性混合独立噪声，绿为基准 1.0）
    float base = grainNoise(p, SEED);
    float rN = mix(grainNoise(p, SEED + 0.1), base, uGrainCorrelation) * uGrainRedRatio;
    float gN = mix(grainNoise(p, SEED + 0.2), base, uGrainCorrelation);
    float bN = mix(grainNoise(p, SEED + 0.3), base, uGrainCorrelation) * uGrainBlueRatio;

    // 强度 = amount × 影调强度比 × AMP × 色彩保护
    float amp = uGrainAmount * strRatio * AMP * uGrainColorPreserve;
    return clamp(c + vec3(rN, gN, bN) * amp, 0.0, 1.0);
}
...
void main() {
    ...
    // 颗粒
    disp = applyGrain(disp, FlutterFragCoord().xy, uSize);
