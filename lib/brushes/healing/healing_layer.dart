...
class HealingLayerProvider with ShaderCacheMixin implements BrushLayerProvider {
  ...
  bool isActive(AdjustmentParams params) =>
      (params.brushMarks['healing']?.isNotEmpty ?? false);

  @override
  int computeMarksHash(AdjustmentParams params) => hashHealingMarks(
  ...
  Future<BrushLayer> render({
    ...
    final marks =
        (params.brushMarks['healing']?.cast<HealingMark>() ?? const []).where((
          m,
        ) {
